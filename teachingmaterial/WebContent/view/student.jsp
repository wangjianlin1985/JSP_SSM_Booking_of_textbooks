...
/* 	//班级
	...
	$('#serieId').change(function(){
		...
		$.post("tClass/findClass.htm" ,function(data) {
			...
			/* html +="<option value=''>--请选择--</option>"; */
			$.each(obj,function(i,val){ 
				if(parseInt(serieId)==parseInt(val.serieId)){
					if(edit=="edit"){
						if(edit_row.classId == val.id){
							html+="<option value='"+val.id+"'  selected='selected'>"+val.className+"</option>";
						}else{
							html+="<option value='"+val.id+"'>"+val.className+"</option>";	
						}
					}else{
		...
		$('#serieId2').change(function(){
		//班级
		$.post("tClass/findClass.htm" ,function(data) {
			var serieId =$('#serieId2').val();
			if(serieId==""){
				message.alert("请先选择系！");
				return;
			}
